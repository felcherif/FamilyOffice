...
global_metrics <- function(output) {
  output$metrics$effective_tax_rate <-
    output$statement$income_statement$expenses_taxes$income_tax["total", , drop = F] / output$statement$income_statement$income$gross_profit["gross_profit", , drop = F]
  output$metrics$overall_leverage_ratio <-
    output$statement$balance_sheet$liabilities$total["total", , drop = F] / output$statement$balance_sheet$equity$total_WO_DTA["total_WO_DTA", , drop = F]
  ...
  for (portfolio_name in names(output$Portfolio$portfolio))
  {
    if (class(output$Portfolio$portfolio[[portfolio_name]]) == "property_class") {
      ...
      output$metrics$savings <-
        rbind(
          output$Portfolio$portfolio[[portfolio_name]]@statement$income_statement$expenses_taxes$savings["savings", ],
          output$metrics$savings
        )

      output$metrics$cash <-
        rbind(
          output$Portfolio$portfolio[[portfolio_name]]@statement$balance_sheet$assets$cash["total", ],
          output$metrics$cash
        )

      output$metrics$operating_cash_flows <-
        rbind(
          output$Portfolio$portfolio[[portfolio_name]]@statement$cash_flow_statement$operating_activities$operating_cash_flows["total", ],
          output$metrics$operating_cash_flows
        )

      output$metrics$financing_cash_flows <-
        rbind(
          output$Portfolio$portfolio[[portfolio_name]]@statement$cash_flow_statement$financing_activities$financing_cash_flows["total", ],
          output$metrics$financing_cash_flows
        )

      output$metrics$increase_in_cash <-
        rbind(
          output$Portfolio$portfolio[[portfolio_name]]@statement$cash_flow_statement$total$increase_in_cash["increase_in_cash", , drop = FALSE],
          output$metrics$increase_in_cash
        )
      ...
      output$metrics$equity_available_for_refinancing <-
        rbind(
          pmax(
            output$Portfolio$portfolio[[portfolio_name]]@refinancing_loan_to_value_max -
              (
                ...
                output$Portfolio$portfolio[[portfolio_name]]@statement$balance_sheet$assets$property[portfolio_name, ],
            0
          ) *
            output$Portfolio$portfolio[[portfolio_name]]@statement$balance_sheet$assets$property[portfolio_name, ],
          output$metrics$equity_available_for_refinancing
      ...
      # loan to value - I
      output$metrics$loan_to_value <- rbind(
        (
          output$Portfolio$portfolio[[portfolio_name]]@statement$balance_sheet$liabilities$mortgage[portfolio_name, ] +
            output$Portfolio$portfolio[[portfolio_name]]@statement$balance_sheet$liabilities$HELOC_balance[portfolio_name, ] +
            output$Portfolio$portfolio[[portfolio_name]]@statement$balance_sheet$liabilities$refinancing_balance[portfolio_name, ]
  ...
  output$metrics$savings <-
    metric_total(output$metrics$savings)
  output$metrics$cash <-
    metric_total(output$metrics$cash)
  output$metrics$operating_cash_flows <-
    metric_total(output$metrics$operating_cash_flows)
  output$metrics$financing_cash_flows <-
    metric_total(output$metrics$financing_cash_flows)
  output$metrics$increase_in_cash <-
    metric_total(output$metrics$increase_in_cash)
  output$metrics$equity_WO_DTA_change <-
    metric_total(output$metrics$equity_WO_DTA_change)
  output$metrics$equity_available_for_refinancing <-
    metric_total(output$metrics$equity_available_for_refinancing)
